    ...
    <section style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 4, background: W.surface2, borderRadius: 4, padding: 3, width: "fit-content", border: `1px solid ${W.border}` }}>
        {(["instruments", "matrix"] as const).map((v) => (
          <button
            ...
            onClick={() => setView(v)}
            style={{
              padding: "5px 14px", fontSize: 13, borderRadius: 2, border: "none", cursor: "pointer",
              fontWeight: 600,
      ...
      {view === "matrix" ? (
        ...
        <div style={{ background: W.surface, border: `1px solid ${W.border}`, borderRadius: 4, padding: 24, textAlign: "center" }}>
          <div style={{ color: W.text, fontSize: 16, fontWeight: 600, marginBottom: 4 }}>
            No tokens on <code>{instance}</code> yet
          </div>
          <div style={{ color: W.dim, fontSize: 16, marginBottom: 16 }}>
          ...
          <div style={{ color: W.faint, fontSize: 13, marginTop: 14 }}>
            ...
            {active && (() => {
              ...
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <span style={{ color: W.dim, fontFamily: wMono, fontSize: 13 }}>
                    {sym} · {active.standard}
                  </span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, color: W.dim, fontSize: 13, marginTop: 4, fontFamily: wMono }}>
                  <span>admin {partyLabel(aliases, active.admin)}</span>
                  <span>· id</span>
                  <MonoId value={active.instrument_id} size={13} color={W.dim} />
                </div>
                ...
                {detailTab === "overview" && (
                  <>
                    ...
                    <h4 style={{ color: W.text2, margin: "16px 0 8px" }}>
                      Holdings <span style={{ color: W.dim, fontWeight: 400, fontSize: 13 }}>· a balance sums its Holding contracts. Click a row to expand.</span>
                    </h4>
                    ...
                    {holdingsErr && <div role="alert" style={{ color: W.err, fontSize: 13 }}>{holdingsErr}</div>}
                          ...
                          <>
                            ...
                            {expanded === h.party && contracts.map((c) => (
                              <tr key={c.contract_id} style={{ background: W.bg }}>
                                <td style={{ ...td, paddingLeft: 34, fontSize: 13 }}>
                                  <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                                    <span style={{ color: W.dim, fontFamily: wMono }}>└</span>
                                    <MonoId value={c.contract_id} size={13} color={W.mag} />
                                    {c.locked && (
                                      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, color: W.warn, fontSize: 11, border: `1px solid ${tint(W.warn, 34)}`, background: tint(W.warn, 13), borderRadius: R.control, padding: "0 5px", height: 16 }}>
                            ...
                            {expanded === h.party && contracts.length === 0 && (
                              <tr><td colSpan={2} style={{ ...td, paddingLeft: 34, color: W.dim, fontSize: 13 }}>Loading contracts…</td></tr>
                            )}
                          </>
                        ))}
                        {holdings.length === 0 && (
  ...
  return (
    <ModalShell title={`Transfer ${symbol}`} onClose={onClose}>
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 10 }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, color: W.text2, fontSize: 13, cursor: "pointer" }}>
        ...
        {plan && (
          <div style={{ background: W.surface2, border: `1px solid ${W.border}`, borderRadius: 4, padding: "10px 12px" }}>
            <div style={{ color: W.dim, fontSize: 13, fontWeight: 500, marginBottom: 6 }}>
              Coin selection preview
            </div>
            {plan.sufficient ? (
              <div style={{ display: "grid", gap: 4, fontFamily: wMono, fontSize: 13 }}>
                {plan.inputs.map((i) => (
                  <div key={i.contract_id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, color: W.text2 }}>
                      <MonoId value={i.contract_id} size={13} color={W.text2} /> consume
                    </span>
              ...
              <div style={{ color: W.warn, fontSize: 13 }}>
  ...
  return (
    ...
    >
      {cards.map((c) => (
        ...
        >
          <div style={{ ...wideCaps, color: W.dim, fontSize: 13, marginBottom: 6 }}>
            {c.label}
          </div>
          <div
          ...
          {c.hint && <div style={{ color: W.dim, fontSize: 11, marginTop: 2 }}>{c.hint}</div>}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <>
      <h4 style={{ color: W.text2, margin: "16px 0 6px" }}>
        Holder distribution{" "}
        <span style={{ color: W.dim, fontWeight: 400, fontSize: 13 }}>· share of total supply</span>
      </h4>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 16 }}>
        <thead>
          <tr style={{ color: W.dim, textAlign: "left" }}>
              ...
              <tr key={h.party}>
                ...
                <td style={td}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontFamily: wMono, fontVariantNumeric: "tabular-nums", color: W.text2, fontSize: 13, minWidth: 44, textAlign: "right" }}>
                      {h.pct_of_supply}%
                    </span>
                  </div>
                </td>
...
function ActivityFeed({ events, err, aliases }: { events: ActivityEvent[] | null; err: string | null; aliases: AliasMap }) {
  if (err) return <div role="alert" style={{ color: W.err, fontSize: 16, marginTop: 12 }}>{err}</div>;
  if (events === null) return <div style={{ color: W.dim, fontSize: 16, marginTop: 12 }}>Scanning ledger history…</div>;
  if (events.length === 0)
    return <div style={{ color: W.dim, fontSize: 16, marginTop: 12 }}>No activity for this instrument yet.</div>;
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 16, marginTop: 12 }}>
      <thead>
        <tr style={{ color: W.dim, textAlign: "left" }}>
          <th style={th}>TIME</th>
          <th style={th}>KIND</th>
      ...
      <tbody>
        {events.map((e) => (
          <tr key={e.offset}>
            <td style={{ ...td, color: W.dim, fontSize: 13, fontFamily: wMono, fontVariantNumeric: "tabular-nums" }}>
...
function MatrixLens({ matrix, err, aliases }: { matrix: BalanceMatrix | null; err: string | null; aliases: AliasMap }) {
  if (err) return <div role="alert" style={{ color: W.err, fontSize: 16 }}>{err}</div>;
  if (!matrix) return <div style={{ color: W.dim, fontSize: 16 }}>Scanning ACS…</div>;
  ...
  return (
    <div style={{ background: W.surface, border: `1px solid ${W.border}`, borderRadius: 4, padding: 16, overflowX: "auto" }}>
      <div style={{ color: W.dim, fontSize: 13, marginBottom: 10 }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 16 }}>
        ...
        <tbody>
          ...
          <tr>
            <td style={{ ...td, color: W.dim, fontWeight: 600, fontSize: 13 }}>Σ total</td>
            {syms.map((s) => (
...
function Header({ right }: { right?: React.ReactNode }) {
  return (
    <header style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <span style={{ color: W.dim, fontSize: 13 }}>Token Standard instruments + actions</span>
...
function PartyManagerModal({
  ...
  return (
    <ModalShell title="Parties" onClose={onClose}>
      <p style={{ color: W.dim, fontSize: 13, marginTop: 0 }}>
      ...
      <form onSubmit={add} style={{ display: "flex", gap: 8 }}>
        <input
          ...
          style={{ flex: 1, background: W.bg, border: `1px solid ${W.border}`, borderRadius: 2, padding: "8px 10px", color: W.text, fontSize: 16 }}
        />
  ...
  return (
    <ModalShell title="Create V2 instrument" onClose={onClose}>
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 8 }}>
        ...
        {err && <div role="alert" style={{ color: W.err, fontSize: 13 }}>{err}</div>}
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 6 }}>
          <Button variant="ghost" size="md" onClick={onClose}>Cancel</Button>
  ...
  if (mode === "create") {
    return (
      <div style={{ display: "grid", gap: 6 }}>
        ...
        {err && <span role="alert" style={{ color: W.err, fontSize: 13 }}>{err}</span>}
        <Button
          variant="ghost"
          size="sm"
    ...
    return (
      <div style={{ display: "grid", gap: 4 }}>
        <input
          ...
          style={{ ...input, fontFamily: wMono, fontSize: 13 }}
        />
        {all.length > 0 && (
          <Button
            variant="ghost"
...
const input: React.CSSProperties = {
  ...
  borderRadius: 2, padding: "6px 8px", fontSize: 16,
};
const th: React.CSSProperties = { ...tableCaps, padding: "6px 10px", borderBottom: `1px solid ${W.border}`, fontSize: 13 };
...
function notice(tone: "ok" | "warn" | "err"): React.CSSProperties {
  ...
  return {
    ...
    borderRadius: R.control, padding: "8px 12px", fontSize: 13,
  };
}
