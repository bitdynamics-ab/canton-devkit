...
export function Dashboard() {
  ...
  return (
    <div>
      ...
      >
        <h1 style={{ fontSize: fs.h2, fontWeight: 600, marginTop: 0, marginBottom: 0 }}>
          LocalNet instances
        </h1>
        <Button
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    ...
    >
      ...
      <p style={{ marginBottom: 0, fontSize: fs.small }}>
        Or run{" "}
  ...
  return (
    ...
    >
      <header style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
        <h3 style={{ margin: 0, fontSize: fs.body, fontWeight: 600, color: W.text }}>Recent activity</h3>
      ...
      {state.kind === "loading" && (
        <div style={{ color: W.dim, fontSize: fs.body, padding: "8px 0" }}>Scanning recent ledger updates…</div>
      )}
      {state.kind === "needs-jwt" && (
        <div style={{ color: W.dim, fontSize: fs.small, padding: "8px 0" }}>
      ...
      {state.kind === "ok" && events.length > 0 && (
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: fs.body, marginTop: 8 }}>
          <thead>
          ...
          <tbody>
            {events.map((e) => (
              <tr key={e.key} style={{ borderTop: `1px solid ${W.border}` }}>
                <td style={{ ...actTd, color: W.dim, fontFamily: wMono, fontVariantNumeric: "tabular-nums", fontSize: fs.small }}>{e.time}</td>
                <td style={actTd}>
                  <span
                    style={{
                ...
                <td style={actTd}>
                  <MonoId value={e.cid} head={6} tail={6} size={fs.small} color={W.info} />
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {state.kind === "ok" && state.truncated && (
        <div style={{ color: W.dim, fontSize: fs.small, marginTop: 8 }}>
...
const actTh: React.CSSProperties = { ...tableCaps, padding: "6px 10px 6px 0", fontSize: fs.small };
