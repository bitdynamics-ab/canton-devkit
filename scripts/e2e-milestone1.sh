...
# ── Known failures (CLI bugs tracked in Linear) ─────────────────────
# Tests listed here are expected to fail due to known product bugs.
# They are still executed and reported but do not cause a non-zero exit.
declare -A KNOWN_FAIL=(
  ["M1-LOG-001"]="CLI logs command returns empty output on Linux"
  ["M1-DWN-001"]="CLI down does not stop Docker containers on Linux"
  ["M1-CLN-001"]="CLI clean --force does not stop Docker containers on Linux"
)
...
fail() {
  local test_id="${1%%:*}"
  if [[ -n "${KNOWN_FAIL[$test_id]+x}" ]]; then
    FAIL_KNOWN=$((FAIL_KNOWN + 1))
    RESULTS+=("⚠ KNOWN $1  — $2 [known: ${KNOWN_FAIL[$test_id]}]")
    printf '\033[1;33m  ⚠ KNOWN %s — %s\033[0m\n' "$1" "$2"
  else
    FAIL=$((FAIL + 1))
    RESULTS+=("✗ FAIL  $1  — $2")
...
print_summary() {
  ...
  printf ' PASSED: %d  FAILED: %d  KNOWN: %d  SKIPPED: %d\n' "$PASS" "$FAIL" "$FAIL_KNOWN" "$SKIP"
  echo "═══════════════════════════════════════════════════════"
  for r in "${RESULTS[@]}"; do
    echo "  $r"
  done
