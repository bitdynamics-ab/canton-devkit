...
**Per-instance overlay (opt-out).** `up --observability-mode` selects the
...
is reachable; `shared` forces shared-only; `per-instance` forces the
overlay. The mode is persisted, so a re-up preserves it. When the overlay
