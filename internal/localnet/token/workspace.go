...
func RunBalanceMatrix(ctx context.Context, opts BalanceOptions) (*BalanceMatrix, error) {
	...
	if opts.Endpoint == "" {
		...
		// Resolution still came back empty and the caller passed no
		// explicit --endpoint: the instance is absent or stopped. Surface
		// an instance-specific remedy here instead of letting it fall
		// through to dialLedger's generic "ledger endpoint is required
		// (host:port)" guard, mirroring the contracts/ledger_endpoint
		// precedent that names the instance and the up/--endpoint fix.
		if opts.Endpoint == "" {
