...
// CoreServicesFor resolves the core-services list (compose service
// names whose absence collapses the reconciler's evalStatus to
// `failed`) for a given persisted splice_version. Returns nil when the
// version isn't resolvable — callers treat nil as "skip the
// core-services check" to stay compatible with registry entries whose
...
// evaluated; the list depends only on the adapter Major, which
// ResolveForOperation recovers from the resolved cache or the tag.
