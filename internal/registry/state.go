...
//
// On-disk layout under ~/.canton/localnet/:
//
...
type State struct {
	...
	DataDir    string `json:"data_dir"`    // ~/.canton/localnet/<name>/
...
// Defaults to ~/.canton/localnet/. Overridable for tests via CANTON_DEVKIT_REGISTRY.
func Root() string {
	...
	if err != nil {
		return filepath.Join(".", ".canton", "localnet")
	}
	return filepath.Join(home, ".canton", "localnet")
}
