...
// the reviewer flagged: skill docs reference `dpm localnet <verb>`
// commands and `--flag` arguments, and manual review can't keep
// them in sync with the live cobra surface. This test parses every
// fenced `sh` block in skills_embed/*.md, extracts the
...
//     landed yet (dar/contracts/token at the time of writing).
//     Skills documenting future commands MUST cite their ticket
//     ID in the doc; the allowlist's value is that ticket so
//     `grep -rn 'BIT-XXX' internal/cli/localnet/skills_lint_test.go`
...
//     for the next fenced block (e.g. shell utility examples
//     that don't reference dpm flags meaningfully).
//
// Without this test, a future cobra refactor that renames
// --port to --tcp-port (say) silently breaks every skill that
// teaches an agent the old flag — and the agent then teaches
// the user the broken flag. The test fails CI on the next push.
func TestSkillsLint_AgainstLiveCobraSurface(t *testing.T) {
	root := Build() // `dpm localnet` root
	cmds, err := skillCmds(skillsFS, skillsEmbedRoot)
	if err != nil {
	...
	for _, sc := range cmds {
		t.Run(sc.location, func(t *testing.T) {
			...
			// flag-check skip too. The branched-PR delivery model
			// means status/down/clean/logs land per-branch as full
			// implementations; while a stub is the cobra surface,
			// validating flags would force the skill to track the
			// transitional-stub shape instead of the final shape
			// users see. Same logging contract as futureVerbs so
			// CI output makes the deferred verification visible.
...
// follow-ups are grep-able:
//
//	grep -rn 'BIT-1' internal/cli/localnet/skills_lint_test.go
//
...
var futureVerbs = map[string]string{
	"dar":       "BIT-127", // dar upload/diff/watch/list — PR #17 branch
	"contracts": "BIT-136", // contracts acs/tx/get
	"token":     "BIT-138", // token create/mint/transfer/burn/balance
	// Verbs not in the cobra tree on this branch (will be added
	// by their per-ticket PRs; once those merge into main, remove
	// the entry and the lint validates flags against the real
	// implementation):
	"doctor": "BIT-123", // p1-05-implement-localnet-doctor
	"list":   "BIT-146", // p1-08-localnet-list
	"env":    "BIT-125", // p1-09-implement-localnet-env
	"creds":  "BIT-141", // umbrella — credential-printing command
	// Live verbs on this branch: up / down / restart / clean /
	// status / logs / versions / ui / skills. Stubs among these
	// (DisableFlagParsing=true) get a flag-check skip via the
	// stub handling above; once their real impls land per-branch,
	// the lint validates flags automatically without changes here.
}
...
// dpmInvocationRE matches the start of a `dpm localnet ...` line
// in a fenced sh block. The capture is the rest of the line after
// "dpm localnet ".
var dpmInvocationRE = regexp.MustCompile(`(?m)^\s*dpm\s+localnet\s+(.*)$`)

// shFenceRE matches the opening fence of a ```sh / ```bash / ```shell
// block. We accept all three because skills written by different
// authors use different conventions.
var shFenceRE = regexp.MustCompile("^```(sh|bash|shell)\\s*$")

// flagRE matches `--foo` (with or without `=value` or following
// argument). Used to extract individual flags from the rest of a
// command line. We deliberately ignore single-letter `-f` shorthand
...
// skillCmds walks the embedded skills FS, extracts every
// `dpm localnet ...` invocation from fenced sh blocks, and returns
// them with source locations.
func skillCmds(fsys fs.FS, root string) ([]scannedCommand, error) {
	...
	err := fs.WalkDir(fsys, root, func(path string, d fs.DirEntry, walkErr error) error {
		...
		cmds := extractDPMCommands(string(body))
		base := strings.TrimPrefix(path, root+"/")
		for i := range cmds {
			cmds[i].location = base + cmds[i].location
		}
		out = append(out, cmds...)
		return nil
	})
...
// out every `dpm localnet <verb> ... --flag ...` invocation it
// finds inside fenced sh / bash / shell blocks.
//
// Multi-line commands joined by trailing `\` are collapsed onto
// a single line before flag extraction.
//
// "<!-- skill-lint: skip-next -->" preceding a fenced block opts
// the whole block out of the lint.
...
func parseBlock(lines []string, startLine int) []scannedCommand {
	...
	for i, line := range joined {
		...
		// Verb path is the leading positional args (everything up
		// to the first `--flag`, `<placeholder>`, `$var`, `"quoted"`
		// or end-of-line). We stop at the first token that starts
		// with `-`, `<`, `$`, or `"` to handle the common patterns.
		...
		for _, f := range fields {
			...
			// A bare `--` alone is unlikely; treat any non-flag
			// non-placeholder identifier as a verb segment.
...
// resolveCobra walks the cobra tree following verbPath. Returns the
// final *cobra.Command and true if every segment matched; false if
// any segment didn't find a child.
...
// persistent-flag inheritance) defines the given flag name.
...
// extractDPMCommands. The end-to-end TestSkillsLint test would
// catch regressions in the parser too, but only by failing on
// shipped skill text — a parser bug that skips a fenced block
// would mean the end-to-end test never runs the affected assertion.
// This test feeds known-tricky inputs and asserts the parsed output.
...
// TestSkillsLint_FutureVerbsHaveTicketIDs is the contract pin
// for the futureVerbs map: every entry MUST reference a real
// BIT-NNN ticket so cross-branch follow-ups stay grep-able and
// outstanding skill drift remains visible. Empty values (a
// future verb with no ticket attribution) get rejected.
