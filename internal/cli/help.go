	...
	return []helpCategory{
		{
			Title: "lifecycle",
			Commands: []helpRow{
				{"up", "start a LocalNet instance"},
				{"down", "stop services (keeps volumes)"},
				{"restart", "restart services in place"},
				{"pause", "pause running containers"},
				{"resume", "unpause paused containers"},
				{"clean", "remove instance state, containers, and volumes"},
				{"snapshot", "save volumes and registry state to a tarball"},
				{"restore", "restore state from a snapshot"},
				{"status", "show services, endpoints, and identities"},
				{"logs", "tail or follow container logs"},
				{"env", "print endpoints and credentials as env vars"},
				{"creds", "show ledger API JWTs"},
				{"list", "list LocalNet instances on this host"},
				{"doctor", "check docker, ports, and host resources"},
				{"refresh", "sync registry status with docker"},
			},
		},
		{
			Title: "developing",
			Commands: []helpRow{
				{"versions", "list supported Splice versions"},
				{"ui", "open the web dashboard"},
				{"container", "list, restart, or tail a container"},
				{"contracts", "query the active contract set"},
				{"tx", "list or replay ledger transactions"},
				{"dar", "upload, list, and inspect DAR files"},
				{"token", "create, mint, transfer, and burn tokens"},
				{"metrics", "scrape Prometheus headline metrics"},
				{"observability", "enable or disable Prometheus and Grafana"},
				{"skills", "list and install bundled agent skills"},
			},
		},
	}
}
...
func renderBoxedHelp() string {
	...
	boxBody := []string{
		helpTitle(),
		"start, stop, and develop on",
		"Canton LocalNet instances",
	}
