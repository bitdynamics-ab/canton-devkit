	...
	return []helpCategory{
		{
			...
			Commands: []helpRow{
				...
				{"stop", "gracefully stop a running LocalNet and keep it restartable"},
				{"down", "stop services and remove runtime containers"},
				{"restart", "restart services in place without removing containers"},
				{"pause", "pause a running LocalNet"},
				{"resume", "resume a paused LocalNet"},
				{"remove", "remove instance state, containers, and volumes"},
				{"snapshot", "capture a LocalNet snapshot to a tarball"},
				{"restore", "restore a LocalNet snapshot from a tarball"},
				...
				{"refresh", "refresh saved instance status from running services"},
			},
		},
		{
			Title: "developing",
			Commands: []helpRow{
				{"versions", "list available Splice versions"},
				...
				{"skills", "browse and install workflow guides for DevKit commands"},
			},
		},
	}
}
