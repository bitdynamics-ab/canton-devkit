...
// TestJWT_RedactedByDefault is the when
...
// demos from leaking a usable signing token. Mirrors the existing // --include-jwt convention on the CLI side.
...
// TestJWT_BodyCapEnforced is the the
...
// TestJWT_AuditLogEmittedOnIssue pins the invariant —
...
// TestErrorBody_AlignedWithFriendlyTaxonomy pins // error responses must carry the (Code, Error,
...
// are reflective pins. Reflective assertions
...
// TestAuth_CacheControlNoStore is the every API JSON response carries no-store. The
...
// TestWriteError_5xxDoesNotLeakCausePath pins // the 5xx response body must NOT
